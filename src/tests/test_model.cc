...
TEST(ModelTest, model_test_1) {
  ...
  model.translate(obj, 0.5f, 1.00f, -0.3f);
  for (size_t i = 0; i < 12; ++i) {
    EXPECT_FLOAT_EQ(obj.vertexArray[i], expectedResult[i]);
  }
}

TEST(ModelTest, model_test_2) {
  s21::Model model;
  s21::ObjectData obj = {};
  std::array<float, 12> vertexModel = {0.4f,  1.2f,  0.98f, 1.1f,
                                       1.22f, 1.5f,  0.06f, 0.07f,
...
TEST(ModelTest, model_test_3) {
  ...
  model.scale(obj, 0.5f, 1.00f, -0.5f);
  for (size_t i = 0; i < 12; ++i) {
    EXPECT_FLOAT_EQ(obj.vertexArray[i], expectedResult[i]);
  }
}

TEST(ModelTest, model_test_4) {
  s21::Model model;
  s21::ObjectData obj = {};
  EXPECT_EQ(model.parseObject("models/test3.obj", obj), true);
